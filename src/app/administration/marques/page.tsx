...
export default function MarquesAdminPage() {
  ...
  const handleCreate = () => {
    ...
    if (form.name.trim().toLowerCase() === "autre") { setError("Ce nom est réservé à la marque par défaut"); return; }
    if (brands.some((b) => b.name.toLowerCase() === form.name.trim().toLowerCase())) {
    ...
    <div className="max-w-xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {brands.length === 0
              ? "Aucune marque — ajoutez-en une pour commencer"
        ...
        <button onClick={() => { setShowForm(!showForm); setError(""); setForm(emptyForm); setEditingId(null); }}
          ...
          {showForm ? "Fermer" : "Ajouter une marque"}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-purple-100 shadow-sm p-6 space-y-4">
          <h2 className="font-bold text-gray-900">Nouvelle marque</h2>
          {error && <p className="text-xs text-red-500">{error}</p>}
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-1.5">Nom de la marque *</label>
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              onKeyDown={(e) => e.key === "Enter" && handleCreate()}
              placeholder="ex: Samsung, Bosch, Philips..." autoFocus
          ...
          <ImageUpload label="Logo de la marque" value={form.logo}
            onChange={(b64) => setForm((f) => ({ ...f, logo: b64 }))} />
      ...
      {brands.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
          ...
          <p className="font-semibold text-gray-700">Aucune marque pour le moment</p>
          <p className="text-sm text-gray-400 mt-1">Ajoutez vos premières marques pour les sélectionner lors de l'ajout de produits.</p>
          {!showForm && (
            <button onClick={() => setShowForm(true)}
              ...
              <PlusCircle className="w-4 h-4" /> Ajouter une marque
            </button>
          )}
        </div>
      ) : (
            ...
            <div key={brand.id}>
              {editingId === brand.id ? (
                ...
                <div className="p-5 space-y-4 bg-purple-50/50">
                  ...
                  <ImageUpload label="Logo" value={editForm.logo}
                    onChange={(b64) => setEditForm((f) => ({ ...f, logo: b64 }))} />
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                      {brand.logo
                        ...
                        : <span className="text-sm font-black text-purple-600">{brand.name.slice(0, 2).toUpperCase()}</span>
                      }
                    </div>
                    <div>
                      <span className="font-semibold text-gray-900">{brand.name}</span>
                      {brand.id === "brand-autre" && (
                        <span className="ml-2 text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Par défaut</span>
                      )}
                    </div>
                  </div>
                  {brand.id === "brand-autre" ? (
                    <span className="text-xs text-gray-400 italic pr-1">Non modifiable</span>
                  ) : (
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setToDelete(brand.id)}
      ...
      <ConfirmDialog
        ...
        message="Cette marque sera retirée du catalogue. Les produits associés ne seront pas supprimés."
        confirmLabel="Supprimer"
        onConfirm={() => { if (toDelete) { deleteBrand(toDelete); setToDelete(null); } }}
